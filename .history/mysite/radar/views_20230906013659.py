    ...
    if request.method == "POST":
        ...
        if 'generate' in request.POST:
            ...
            if form.is_valid():
                ...
                if request.POST['position_choice'] == 'top':
                    top = crc.TopRadar(league, split, min_game_count)
                    top.create_radar()
                elif request.POST['position_choice'] == 'jungle':
                    jng = crc.JngRadar(league, split, min_game_count)
                    jng.create_radar()
                elif request.POST['position_choice'] == 'mid':
                    mid = crc.MidRadar(league, split, min_game_count)
                    mid.create_radar()
                elif request.POST['position_choice'] == 'bot':
                    bot = crc.BotRadar(league, split, min_game_count)
                    bot.create_radar()
                elif request.POST['position_choice'] == 'support':
