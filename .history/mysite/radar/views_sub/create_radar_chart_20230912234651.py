...
class TopRadar(TopDataFrame):
    ...
    def create_radar(self, file_name=str(DT_NOW.strftime('%m')) + str(DT_NOW.strftime('%d'))):
        ...
        for p in glob.glob(f'{IMG_PATH}radar_image*.png'):
            if os.path.isfile(p):
                os.remove(p)
        # os.remove(glob.glob('radar/static/radar/images/radar_image??????.png'))
...
class MidRadar(MidDataFrame):
    ...
    def __init__(self, league, split, min_game_count=MIN_GAME_COUNT, playoffs=False):
        ...
        self.rnd = str(random.randint(0, 100000))
    ...
    def create_radar(self, file_name=str(DT_NOW.strftime('%m')) + str(DT_NOW.strftime('%d'))):
        ...
        fig.savefig(f'radar/static/radar/images/radar_image{self.rnd}.png', bbox_inches=None)
        # plt.show()
    ...
    def create_radar(self, file_name=str(DT_NOW.strftime('%m')) + str(DT_NOW.strftime('%d'))):
        ...
        if len(self.players) <= 11 or 13 <= len(self.players) < 16:
            fig.text(0.77, 0.05,
                    ...
                    'VSPM: Vision Score Per Minute',
                    horizontalalignment='left', color='black', size='16')
        # fig.savefig(f'2023{self.league}_{self.split}_sup{file_name}.png', bbox_inches=None)
        rnd = str(random.randint(0, 100000))
        fig.savefig(f'radar/static/radar/images/radar_image{rnd}.png', bbox_inches=None)
        # plt.show()
