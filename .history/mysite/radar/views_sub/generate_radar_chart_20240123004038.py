...
class DataProcessing(Preprocess):
    ...
    def create_solokill(self):
        # top_players = self.data.make_groups().query(
        #     '(position == "top") & (teamname != "" "")'
        #     ).index.get_level_values('playername').sort_values(key=lambda x: x.str.lower())
        # jng_players = self.data.make_groups().query(
        #     '(position == "jng") & (teamname != "" "")'
        #     ).index.get_level_values('playername').sort_values(key=lambda x: x.str.lower())
        # mid_players = self.data.make_groups().query(
        ...
        # solokill.index.set_names('playername', inplace=True)
        # # mid_solokill.index.set_names('playername', inplace=True)
        # steal.index.set_names('playername', inplace=True)

        # df = self.data.make_groups()
        # df = df.join(solokill)
        # df = df.join(steal)
    ...
    def calc_average(self):
        ...
        # 最少ゲーム数を設定
        # df_average = df_average[df_average['games_played'] >= self.min_game_count]
...
class RadarProcessing:
    ...
    def __init__(self, league, split, min_game_count=MIN_GAME_COUNT):
        ...
        self.df_base = pd.read_csv(
            f'{self.league}_{self.split}_processing.csv', index_col=['playername', 'teamname', 'position'])
...
class GenerateRadar(RadarProcessing):
    ...
    def __init__(self, league, split, position, min_game_count=MIN_GAME_COUNT):
        ...
        # def validate_min_game_count(count):
        #     if count > 8:
        #         raise ValueError('min_game_count must be less than 8')
        # try:
        #     validate_min_game_count(self.min_game_count)
        ...
        # self.df_avg = self.get_df().xs(self.position, level='position').copy()
        ...
        # radar_chart.pyのdata形式に合わせる
        # for key, value in self.df_std.index:
        #     self.team_dict[key] = value
        # # プレイヤー名とチーム名の辞書を作成
        # self.players = self.df_std.reset_index('teamname').index
        # # top_playersだとAverageがないので新しく作成
        # temp_values = self.df_std.values.tolist()
        # self.plot_values = [[temp_values[i], temp_values[-1]] for i in range(len(temp_values))]
        # self.plot_labels = self.create_labels().xs(self.position, level='position').values.tolist()
        # # radar_chart.pyのdata形式に合わせる

    # def calc_std(self):
    #     # rounded前のdfを標準化して偏差値に変換
    #     # レーダーチャートのプロットは偏差値でおこなう
    #     # df_avg = self.df.xs(self.position, level='position').copy()
    #     df_std = stats.zscore(self.df_avg, axis=0).apply(lambda x: x*10 + 50)
    #     return df_std
    ...
    def generate_radar(self, file_name=str(DT_NOW.strftime('%m%d'))):
        ...
        fig.suptitle(
            x=0.5, y=0.97, t=f'{DT_NOW.strftime("%Y")} {self.league} {self.split} {self.position.upper()} Stats',
...
# %%
# RadarProcessing('LEC', 'Winter').df_base.isna().sum().sum()
# RadarProcessing('LEC', 'Spring').df_base.isna().sum().sum()
# RadarProcessing('LEC', 'Summer').df_base.isna().sum().sum()
# RadarProcessing('LCK', 'Spring').df_base.isna().sum().sum()
# RadarProcessing('LCK', 'Summer').df_base.isna().sum().sum()
# RadarProcessing('LJL', 'Spring').df_base.isna().sum().sum()
# RadarProcessing('LJL', 'Summer').df_base.isna().sum().sum()
...
# %%
# RadarProcessing('LCK', 'Summer', 3).get_df()

# %%
hoge = GenerateRadar('LJL', 'Spring', 'sup', 3)
hoge.generate_radar()
