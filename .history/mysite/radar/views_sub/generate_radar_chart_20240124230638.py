    ...
    def generate_radar(self, file_name=str(DT_NOW.strftime('%m%d'))):
        ...
        if len(self.players) >= 13:
            fig, axs = plt.subplots(
                figsize=(8, 9), nrows=4, ncols=4, subplot_kw=dict(projection='radar'), facecolor='whitesmoke')
            fig.subplots_adjust(wspace=0.6, hspace=0.24, top=0.88, right=0.95, bottom=0, left=0.05)
        else:
            fig, axs = plt.subplots(
                figsize=(8, 8), nrows=3, ncols=4, subplot_kw=dict(projection='radar'), facecolor='whitesmoke')
